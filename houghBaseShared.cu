...
__global__ void GPU_HoughTran(unsigned char *pic, int w, int h, int *acc, float rMax, float rScale) {
    ...
    // 1. Declaración de Acumulador Local en Memoria Compartida
    __shared__ int localAcc[degreeBins * rBins];
    
    // 2. Inicialización del Acumulador Local en Memoria Compartida
    for (int i = locID; i < degreeBins * rBins; i += blockDim.x) {
        localAcc[i] = 0;
    }
    // 3. Sincronización de los hilos después de la inicialización
    __syncthreads();
    ...
    if (pic[gloID] > 0) {
        for (int tIdx = 0; tIdx < degreeBins; tIdx++) {
            ...
            //4. Modificacion en la actualizacion del acumulador local
            atomicAdd(&localAcc[rIdx * degreeBins + tIdx], 1);
        }
    }
    // 5. Segunda barrera de sincronización
    __syncthreads();

    //6.  Bucle para sumar valores locales al acumulador global
    for (int i = locID; i < degreeBins * rBins; i += blockDim.x) {
